...
export function AppThemeProvider({ children }: { children: React.ReactNode }): JSX.Element {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const nextPreference = raw && isThemePreference(raw) ? raw : "system";
        setThemePreferenceState(nextPreference);
        try {
          setColorScheme(nextPreference);
        } catch (err) {
          console.warn("[OV:theme] failed to apply loaded theme preference:", err);
        }
      } catch (err) {
        if (cancelled) return;
        console.warn("[OV:theme] failed to load theme preference:", err);
        setThemePreferenceState("system");
        try {
          setColorScheme("system");
        } catch (applyErr) {
          console.warn("[OV:theme] failed to apply fallback theme preference:", applyErr);
        }
      }
    })();
    return () => {
  ...
  const setThemePreference = useCallback((preference: ThemePreference) => {
    ...
    try {
      ...
      console.warn("[OV:theme] failed to apply selected theme preference:", err);
    }
    AsyncStorage.setItem(STORAGE_KEY, preference).catch((err) => {
      console.warn("[OV:theme] failed to persist theme preference:", err);
    });
  }, [setColorScheme]);
