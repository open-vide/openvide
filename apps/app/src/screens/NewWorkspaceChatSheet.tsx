...
type Props = NativeStackScreenProps<MainStackParamList, "NewWorkspaceChatSheet">;
...
export function NewWorkspaceChatSheet({ route, navigation }: Props): JSX.Element {
  ...
  const handleCreate = async (): Promise<void> => {
    ...
    try {
      ...
      navigation.replace("AiChat", { sessionId: session.id, workspaceId: workspace.id });
    } catch (err) {
  ...
  return (
    <ScrollView className="flex-1 bg-card" contentContainerStyle={{ padding: 20, gap: 14 }} showsVerticalScrollIndicator={false}>
