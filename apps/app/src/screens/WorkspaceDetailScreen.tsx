...
function workspaceChatsCacheKey(workspaceId: string): string {
  return `open-vide/workspace-chats/${workspaceId}`;
}
...
function sessionTitle(session: AiSession): string {
  ...
  // Try turn prompts when messages haven't been loaded yet
...
function workspaceChatTitle(chat: WorkspaceChatInfo): string {
  ...
  const lastPrompt = chat.lastTurn?.prompt?.trim();
  if (lastPrompt) return lastPrompt.slice(0, 72);
  return "Untitled chat";
}
...
export function WorkspaceDetailScreen({ route, navigation }: Props): JSX.Element {
  ...
  const refreshInFlightRef = useRef<{ promise: Promise<void>; startedAt: number } | null>(null);

  const workspace = getWorkspace(workspaceId);
  const target = workspace ? getTarget(workspace.targetId) : undefined;

  // Load cached workspace chats from AsyncStorage on mount
  useEffect(() => {
    if (!workspace) return;
    void (async () => {
      try {
        const raw = await AsyncStorage.getItem(workspaceChatsCacheKey(workspace.id));
        if (raw) {
          const cached = JSON.parse(raw) as WorkspaceChatInfo[];
          setWorkspaceChats((prev) => (prev.length === 0 ? cached : prev));
  ...
  const refreshWorkspaceChats = useCallback(async () => {
    if (!workspace) return;

    const existing = refreshInFlightRef.current;
    if (existing) {
      if (Date.now() - existing.startedAt < 15_000) {
    ...
    const entry: { promise: Promise<void>; startedAt: number } = {
      startedAt: Date.now(),
    ...
    entry.promise = (async () => {
      try {
        ...
        void AsyncStorage.setItem(workspaceChatsCacheKey(workspace.id), JSON.stringify(chats));
      } catch (err) {
  ...
  // Fetch on mount and every time the screen regains focus
  useEffect(() => {
    void refreshWorkspaceChats();
  ...
  const handleDeleteWorkspaceChat = useCallback((chat: WorkspaceChatInfo) => {
    ...
    // Remove from local workspace chats state and update cache
    setWorkspaceChats((prev) => {
      const next = prev.filter((c) => c.id !== chat.id);
      void AsyncStorage.setItem(workspaceChatsCacheKey(workspaceId), JSON.stringify(next));
      return next;
    });
  ...
  const handleOpenWorkspaceChat = useCallback(async (chat: WorkspaceChatInfo) => {
    ...
    // Fast path: if we already have a local session for this chat, navigate immediately
    // instead of re-doing the heavy SSH history hydration.
    const existing = allSessions.find((s) =>
      (chat.daemonSessionId && s.daemonSessionId === chat.daemonSessionId) ||
      (chat.resumeId && s.conversationId === chat.resumeId),
    );
    if (existing) {
      navigation.navigate("AiChat", { sessionId: existing.id, workspaceId: workspace.id });
      return;
    }
  ...
  }, [navigation, openWorkspaceChat, workspace, allSessions]);
    ...
    <View className="flex-1 bg-background">
      ...
      {loadingChats && (
        <View className="flex-row items-center justify-center gap-2 py-2 border-b border-border">
          ...
          <Text className="text-dimmed text-[12px]">Refreshing chats...</Text>
        </View>
      )}
