...
export function DirectoryPicker({ route, navigation }: Props): JSX.Element {
  ...
  const promptCreateDirectory = useCallback(() => {
    if (!currentDir) return;
    Alert.prompt("New Folder", "Enter folder name:", async (name) => {
      if (!name?.trim()) return;
      try {
        await browserRef.current?.createDirectory(currentDir, name.trim());
        void loadEntries(currentDir);
      } catch (err) {
  ...
  ], [toggleSearch, promptCreateDirectory]);
  ...
  const handleSelect = useCallback(() => {
    if (!currentDir) return;
    navigation.navigate(returnTo ?? "NewSessionSheet", {
      selectedDirectory: currentDir,
      ...(route.params.returnState ?? {}),
    } as never);
  }, [currentDir, navigation, returnTo, route.params.returnState]);
