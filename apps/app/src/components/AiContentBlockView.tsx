...
export const AiContentBlockView = React.memo(function AiContentBlockView({
  ...
  const shouldHideToolResult = (): boolean => {
    if (block.isError) {
      return false;
    }
    if (block.toolName === "Read" || block.toolName === "Grep" || block.toolName === "Glob") {
      return true;
    }
    if (block.toolName === "exec_command") {
      ...
      return intent.kind === "read" || intent.kind === "search" || intent.kind === "list";
    }
    return false;
  };
  ...
  switch (block.type) {
    ...
    case "command_exec": {
      const intent = describeCommandIntent(block.command ?? "");
      const isRunning = isStreaming === true && isLastBlock === true && !block.output && block.exitCode == null;

      if (intent.kind === "read" && intent.filePath) {
        return (
          <ToolActivityRow
            icon="file-text"
            label={`Read ${basename(intent.filePath)}`}
            status={isRunning ? "running" : (block.exitCode != null && block.exitCode !== 0 ? "error" : "completed")}
          />
        );
      }
      ...
      if (intent.kind === "search" && intent.pattern) {
        const label = intent.path
          ? `Search ${intent.pattern} in ${intent.path}`
      ...
      if (intent.kind === "list") {
        return (
          <ToolActivityRow
            icon="folder"
            label={intent.path ? `List ${intent.path}` : "List files"}
            status={isRunning ? "running" : (block.exitCode != null && block.exitCode !== 0 ? "error" : "completed")}
          />
        );
      }
