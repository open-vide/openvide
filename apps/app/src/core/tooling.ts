...
export function buildDaemonScript(action: DaemonAction): string {
  switch (action) {
    case "install":
      return [
        "set -e",
        "echo 'STEP 1/6: Checking node/npm prerequisites'",
        nodePrereqScript(),
        "echo 'STEP 2/6: Stopping running openvide-daemon'",
        "openvide-daemon stop 2>/dev/null || true",
        "echo 'STEP 3/6: Installing openvide-daemon@latest'",
        "npm install -g openvide-daemon@latest",
        "echo 'STEP 4/6: Verifying openvide-daemon binary'",
        "which openvide-daemon",
        "echo 'STEP 5/6: Verifying openvide-daemon version'",
        "openvide-daemon version",
        "echo 'STEP 6/6: Checking daemon health'",
