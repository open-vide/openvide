...
export class NativeSshClient {
  ...
  private async getOrCreateSession(
    ...
    if (existing && existing.signature === signature) {
      __DEV__ && console.log(`[OV:ssh] getOrCreateSession REUSE target=${target.id.slice(0, 12)} shellReady=${existing.shellReady} hasActive=${!!existing.active}`);
      return existing;
    }
    if (existing) {
      __DEV__ && console.log(`[OV:ssh] getOrCreateSession RECONNECT (signature changed) target=${target.id.slice(0, 12)}`);
    ...
    if (inFlight) {
      __DEV__ && console.log(`[OV:ssh] getOrCreateSession WAIT (connection in-flight) target=${target.id.slice(0, 12)}`);
      return await inFlight;
    }

    __DEV__ && console.log(`[OV:ssh] getOrCreateSession NEW target=${target.id.slice(0, 12)}`);
    const t0 = Date.now();
    const sessionPromise = this.createSession(target, credentials)
      .then((session) => {
        __DEV__ && console.log(`[OV:ssh] getOrCreateSession CONNECTED target=${target.id.slice(0, 12)} +${Date.now() - t0}ms`);
        this.sessions.set(target.id, session);
        this.connecting.delete(target.id);
        return session;
      })
      .catch((error) => {
        __DEV__ && console.log(`[OV:ssh] getOrCreateSession FAILED target=${target.id.slice(0, 12)} +${Date.now() - t0}ms err=${error instanceof Error ? error.message : String(error)}`);
        this.connecting.delete(target.id);
        throw error;
      });
  ...
  async dispose(): Promise<void> {
    ...
    __DEV__ && console.log(`[OV:ssh] dispose: clearing ${targetIds.size} session(s)`);
  ...
  private async dispatchCommand(
    ...
    __DEV__ && console.log(`[OV:ssh] dispatchCommand: ${script.length} bytes for req=${commandContext.requestId.slice(-8)}`);
  ...
  async runCommand(
    ...
    const cmdSnippet = command.slice(0, 60).replace(/\n/g, "\\n");
    const marker = `__OV_EXIT_${requestId}__`;
    const markerRegex = new RegExp(`${escapeRegExp(marker)}\\s*(\\d+)`);
    const rcStart = Date.now();

    for (let attempt = 1; attempt <= 2; attempt += 1) {
      __DEV__ && console.log(`[OV:ssh] runCommand attempt=${attempt} req=${requestId.slice(-8)} cmd="${cmdSnippet}"`);
      const session = await this.getOrCreateSession(target, credentials);
      if (session.active && !session.active.finished) {
        __DEV__ && console.log(`[OV:ssh] runCommand QUEUED req=${requestId.slice(-8)} behind=${session.active.requestId.slice(-8)}`);
      }
      ...
      try {
        __DEV__ && console.log(`[OV:ssh] runCommand DISPATCHING req=${requestId.slice(-8)} +${Date.now() - rcStart}ms`);
        await this.dispatchCommand(session, commandContext, command);
        __DEV__ && console.log(`[OV:ssh] runCommand DISPATCHED req=${requestId.slice(-8)} +${Date.now() - rcStart}ms (waiting for marker)`);
      } catch (error) {
        __DEV__ && console.log(`[OV:ssh] runCommand DISPATCH_FAIL req=${requestId.slice(-8)} +${Date.now() - rcStart}ms err=${error instanceof Error ? error.message : String(error)}`);
        ...
        if (attempt < 2) {
          __DEV__ && console.log(`[OV:ssh] runCommand RETRY (resetting session) req=${requestId.slice(-8)}`);
