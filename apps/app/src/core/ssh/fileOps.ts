...
function parseLsLine(line: string, basePath: string): RemoteFileEntry | null {
  ...
  // Validate that the first field is a real Unix permission string (e.g. drwxr-xr-x).
  // This filters out echoed shell commands and other PTY noise that happen to have 8+ tokens.
  if (!/^[dlcbps-][rwxsStT-]{9}[.+@]?$/.test(permissions)) return null;
...
export async function createFile(
  ...
  const safePath = path.replace(/'/g, "'\\''");
  await runScriptedCommand(ssh, target, creds, `touch '${safePath}'`, {
    timeoutMs: 5000,
    timeoutMessage: "Creating file timed out",
    signal: options?.signal,
  });
}
...
export async function createDirectory(
  ...
  const safePath = path.replace(/'/g, "'\\''");
  await runScriptedCommand(ssh, target, creds, `mkdir -p '${safePath}'`, {
    timeoutMs: 5000,
    timeoutMessage: "Creating directory timed out",
    signal: options?.signal,
  });
}
