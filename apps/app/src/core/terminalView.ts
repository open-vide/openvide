...
function parseTerminalStream(stream: string): { lines: string[]; attrs: AnsiAttrs[][]; row: number; col: number } {
  ...
  return { lines: state.lines, attrs: state.attrs, row: state.row, col: state.col };
}
...
export function buildColoredReadableDisplay(
  ...
  const output: { text: string; attrs: AnsiAttrs[]; sourceRow: number }[] = [];
  const byKey = new Map<string, number>();
  ...
  for (let i = 0; i < cleanedLines.length; i++) {
    ...
    if (line.trim().length === 0) {
      if (output.length > 0 && output[output.length - 1]!.text !== "") {
        output.push({ text: "", attrs: [], sourceRow: i });
      }
      continue;
    }
    ...
    if (!key) {
      ...
      output.push({ text: line, attrs: rowAttrs, sourceRow: i });
      continue;
    }
    ...
    output[existing] = { text: line, attrs: rowAttrs, sourceRow: i };
  }
  ...
  let cursor: ColoredTerminalDisplay["cursor"];
  const cursorRow = visible.findIndex((entry) => entry.sourceRow === parsed.row);
  if (cursorRow >= 0) {
    cursor = {
      row: cursorRow,
      col: Math.max(0, parsed.col),
    };
  } else if (visible.length > 0 && parsed.row >= (visible[visible.length - 1]?.sourceRow ?? 0)) {
    cursor = {
      row: visible.length - 1,
      col: Math.max(0, parsed.col),
    };
  }
