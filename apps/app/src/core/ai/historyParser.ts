...
import { shouldAutoCompleteToolUse } from "./planMode";
...
function applyContextSnapshot(history: ParsedHistory, input: {
  ...
  const boundedUsed = input.windowTokens != null && Number.isFinite(input.windowTokens) && input.windowTokens > 0
    ...
    : input.usedTokens;
  history.contextUsedTokens = Math.floor(boundedUsed);
...
function parseDaemonHistory(tool: ToolName, lines: string[]): ParsedHistory {
  ...
  for (const raw of lines) {
    ...
    // When history is tail-truncated, output lines may appear before any turn_start.
    // Synthesize an implicit turn so these lines aren't silently dropped.
    if (currentTurn < 0 && (t === "o" || t === "e")) {
...
function extractClaudeBlocks(message: Record<string, unknown>): AiContentBlock[] {
  ...
  for (const item of content) {
    ...
    if (type === "text") {
      ...
      const cleanedResult = sanitizeCodexToolOutput(extractTextFromUnknown(block.content));
      out.push({
        type: "tool_result",
...
function hydrateToolMetadata(history: ParsedHistory): void {
  for (const message of history.messages) {
    const toolById = new Map<string, AiContentBlock>();
    for (const block of message.content) {
      if (block.type === "tool_use" && block.toolId) {
        if (shouldAutoCompleteToolUse(block.toolName)) {
          block.toolStatus = "completed";
        }
        toolById.set(block.toolId, block);
        continue;
      }
      if (block.type === "tool_use" && shouldAutoCompleteToolUse(block.toolName)) {
        block.toolStatus = "completed";
        continue;
      }
      if (block.type === "tool_result" && block.toolId) {
        const source = toolById.get(block.toolId);
        if (!source) {
          continue;
        }
        block.toolName = source.toolName;
        block.toolInput = source.toolInput;
        source.toolStatus = block.isError ? "error" : "completed";
      }
    }
  }
}
